...
class MetricService:
        ...
        try:
            ...
            if cached_data:
                ...
                if 'date' in df.columns:
                    df.rename(columns={'date': 'transaction_datetime'}, inplace=True)
    ...
    def get_holdings_in_currency(self, session: Session, portfolio_id: int, asset_id: int, 
        ...
        if holdings_df.empty:
            return pd.DataFrame(columns=['transaction_datetime', 'portfolio_id', 'asset_id', 'holding_value'])

        # Filter by date range
        holdings_df = holdings_df[
            (holdings_df['transaction_datetime'] >= pd.to_datetime(start_date)) & 
            (holdings_df['transaction_datetime'] <= pd.to_datetime(end_date))
        ]
        ...
        # Your existing vectorized calculation - using transaction_datetime consistently
        df = pd.merge(holdings_df, asset_prices[['transaction_datetime', 'close']], on='transaction_datetime', how='left')
        df = pd.merge(df, currency_prices[['transaction_datetime', 'close']], on='transaction_datetime', how='left', 
        ...
        return df[['transaction_datetime', 'portfolio_id', 'asset_id', 'holding_value']]
    ...
    def get_holdings_in_currency_general(self, portfolio_ids: List[int], asset_ids: List[int], 
        ...
        if all_data:
            ...
            return df.sort_values(['transaction_datetime', 'portfolio_id', 'asset_id']).reset_index(drop=True)
        else:
            return pd.DataFrame(columns=['transaction_datetime', 'portfolio_id', 'asset_id', 'holding_value'])
    ...
    def get_cash_flow(self, session: Session, portfolio_id: int, asset_id: int, 
        ...
        if actions_df.empty:
            return pd.DataFrame(columns=['transaction_datetime', 'portfolio_id', 'asset_id', 'cash_flow_daily', 'cash_flow_cumulative'])

        actions_df = actions_df.sort_values(by='transaction_datetime').reset_index(drop=True)
        ...
        action_start = actions_df['transaction_datetime'].min().date()
        action_end = actions_df['transaction_datetime'].max().date()
        ...
        for curr_id in unique_currencies:
            ...
            if curr_id == currency_id:
                ...
                if curr_id in currency_price_data and not target_currency_prices.empty:
                    ...
                    curr_actions = pd.merge(curr_actions, from_prices[['transaction_datetime', 'close']], 
                                          left_on='transaction_datetime', right_on='transaction_datetime', how='left')
                    curr_actions = pd.merge(curr_actions, target_currency_prices[['transaction_datetime', 'close']], 
                                          left_on='transaction_datetime', right_on='transaction_datetime', how='left', 
                                          suffixes=('_from', '_to'))
                    ...
                    for idx, row in curr_actions.iterrows():
                        try:
                            conversion_rate = self.db_access.get_currency_conversion_on_date(
                                session, row['currency_id'], currency_id, row['transaction_datetime'].date()
                            )
        ...
        actions_df = actions_df.groupby(['transaction_datetime', 'portfolio_id', 'asset_id'])['cash_flow'].sum().reset_index()
        actions_df.rename(columns={'cash_flow': 'cash_flow_daily'}, inplace=True)

        # Create full date range
        date_range = pd.date_range(
            start=max(start_date, actions_df['transaction_datetime'].min().date()), 
            end=min(end_date, datetime.now().date()), 
            freq='D'
        )
        
        df = pd.DataFrame(index=date_range)
        df.index.name = 'transaction_datetime'
        
        df = df.merge(actions_df[['transaction_datetime', 'portfolio_id', 'asset_id', 'cash_flow_daily']], 
                     left_index=True, right_on='transaction_datetime', how='left')
        ...
        return df[['transaction_datetime', 'portfolio_id', 'asset_id', 'cash_flow_daily', 'cash_flow_cumulative']]
    ...
    def get_cost_basis_simple(self, session: Session, portfolio_id: int, asset_id: int, currency_id: int,
        ...
        if actions_df.empty:
            return pd.DataFrame(columns=['transaction_datetime', 'portfolio_id', 'asset_id', 'cost_basis', 'cost_basis_value', 'asset_quantity'])
        ...
        actions_agg_df = actions_df.groupby(['transaction_datetime', 'portfolio_id', 'asset_id'])[
        ...
        if not actions_agg_df.empty:
            full_date_range = pd.date_range(
                start=actions_agg_df['transaction_datetime'].min().date(), 
            ...
            df = df.merge(actions_agg_df[['transaction_datetime', 'portfolio_id', 'asset_id', 'cost_basis', 'cost_basis_value', 'asset_quantity']], 
                         left_index=True, right_on='transaction_datetime', how='left')
            ...
            df = df[(df['transaction_datetime'] >= pd.to_datetime(start_date)) & (df['transaction_datetime'] <= pd.to_datetime(end_date))]
            
            return df[['transaction_datetime', 'portfolio_id', 'asset_id', 'cost_basis', 'cost_basis_value', 'asset_quantity']]
    ...
    def get_unrealized_gain_loss(self, portfolio_id: int, asset_id: int, currency_id: int,
        ...
        df = pd.merge(cost_basis_df, holdings_df, on=['transaction_datetime', 'portfolio_id', 'asset_id'], how='outer')
        df = df.sort_values('transaction_datetime').ffill()
        ...
        return df[['transaction_datetime', 'portfolio_id', 'asset_id', 'holding_value', 'cost_basis_value', 
    ...
    def get_time_weighted_return_general(self, portfolio_ids: List[int], asset_ids: List[int], 
        ...
        if holdings_value.empty or cash_flow.empty:
            return pd.DataFrame(columns=['transaction_datetime', 'daily_adjusted_return', 'time_weighted_return'])

        # Merge and aggregate
        data = pd.merge(cash_flow, holdings_value, on=['transaction_datetime', 'portfolio_id', 'asset_id'], how='outer')
        ...
        return data_agg[['transaction_datetime', 'daily_adjusted_return', 'time_weighted_return']]
    ...
    def get_sharpe_ratio(self, portfolio_ids: List[int], asset_ids: List[int], currency_id: int,
        ...
        if twr_df.empty or len(twr_df) < rolling_window_periods:
            return pd.DataFrame(columns=['transaction_datetime', 'rolling_sharpe_ratio'])
        ...
        df = pd.DataFrame({
            'transaction_datetime': twr_df['transaction_datetime'],
            'rolling_sharpe_ratio': sharpe_ratio
        })

        return df[['transaction_datetime', 'rolling_sharpe_ratio']].dropna()

    def aggregate_on_date(self, df: pd.DataFrame, columns: List[str]) -> pd.DataFrame:
        """Aggregate dataframe by date with error handling"""
        if df.empty:
            return pd.DataFrame(columns=['transaction_datetime'] + columns)
        
        try:
            aggregated_df = df.groupby('transaction_datetime')[columns].sum().reset_index()
